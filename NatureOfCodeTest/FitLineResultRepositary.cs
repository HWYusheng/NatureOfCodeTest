...
namespace NatureOfCodeTest
{
    ...
    public class FitLineResultRepositary
    {
        ...
        {
            // UserID in Simulations is the FK linking to Users.UserID
            string sql = "INSERT INTO Simulations (PlanetID, FitScore, TimeTakenSec, UserID) VALUES (?, ?, ?, ?)";
        ...
        public List<FitLineResult> GetAllResults()
        {
            ...
            string sql =
                ...
                "  LEFT JOIN Users u ON s.UserID = u.UserID) " +   // FK link
                    ...
                    using (OleDbDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            results.Add(new FitLineResult
                            {
                                ...
                                UserID       = reader.IsDBNull(5) ? -1        : reader.GetInt32(5),
        ...
        {
            string whereClause = UserSession.IsLoggedIn
                ? "WHERE UserID = " + UserSession.CurrentUserID
